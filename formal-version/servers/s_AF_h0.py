...
# Test Mode:    Listen: localhost:9013
# Otherwise:    Listen: 10.0.3.7:8080
TEST_MODE = True if len(sys.argv) == 2 and sys.argv[1] == '--test' else False
LISTEN_PORT = 9013 if TEST_MODE else 8080
...
def log(text):
    with open("logs/AF.txt", "a") as f:
        f.write(str(text) + "\n")


class SBAAFConnection(object):
    ...
    def resource_providing(self):
        ...
        log(body_json)
        self.rx_config = str(body_json['MODE']['AF'])
        self.fw_body = {
            'RESULT': body_json['RESULT']
        }
        ents = [str(x) for x in body_json['SBA_ENTITY']]
        ents.pop(0)  # delete AF in entities list
        confs = {str(k): str(v) for k, v in body_json['MODE'].items()}
        confs.pop('AF', None)  # delete AF in config table
        ...
        self.fw_body['RESULT']['AF'] = True  # Always gives True for testing
        self.lock.acquire()
    ...
    def send_response(self):
        body = json.dumps(
            {'Content': 'Recieved by SBAES - AF'}).encode('utf-8')
        self.conn.send_headers(
            stream_id=self.stream_id,
            headers=[
                (':status', '200'),
                ('server', 's_AF-h2-server/1.0'),
...
log('SBA Entity AF server started at http://{}:{}'.format(
    '0.0.0.0' if TEST_MODE else '10.0.3.7', LISTEN_PORT))
...
while True:
    try:
        connection = SBAAFConnection(sock.accept()[0], TEST_MODE, LOCK)
