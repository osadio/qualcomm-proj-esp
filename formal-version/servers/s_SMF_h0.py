...
# Test Mode:    Listen: localhost:9011
# Otherwise:    Listen: 10.0.3.5:8080
TEST_MODE = True if len(sys.argv) == 2 and sys.argv[1] == '--test' else False
LISTEN_PORT = 9011 if TEST_MODE else 8080
...
class SBASMFConnection(object):
    ...
    def send_response(self):
        body = json.dumps(
            {'Content': 'Recieved by SBAES - SMF'}).encode('utf-8')
        self.conn.send_headers(
            stream_id=self.stream_id,
            headers=[
                (':status', '200'),
                ('server', 's_SMF-h2-server/1.0'),
    ...
    def foward_request(self):
        fw_table = 'localhost:9002' if self.TEST_MODE else '10.0.4.1:8080'

        send_conn = HTTP20Connection(fw_table)
        fw_body = json.dumps(self.fw_body).encode('utf-8')
        fw_header = dict(self.rx_headers)
        fw_header['plane'] = 'control'
