...
def send_packet(pkt_cnt):
    print('Sending mMTC packet {:0>4d} into system\nheaders: {}'.format(pkt_cnt, headers))
    conn = HTTP20Connection('{}:{}'.format(HOST, SEND_PORT))

    # Add SHA256 encrypted id to header i.e. sha256('mMTC-PKT-0132')
    headers['id'] = hashlib.sha256('mMTC-PKT-{:0>4d}'.format(pkt_cnt)).hexdigest()
    # headers['id'] = 'mMTC-PKT-{:0>4d}'.format(pkt_cnt)
    ...
    print('Response of mMTC packet {:0>4d}:'.format(pkt_cnt))
