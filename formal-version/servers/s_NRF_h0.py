...
# Test Mode:    Listen: localhost:9007
# Otherwise:    Listen: 10.0.3.1:8080
TEST_MODE = True if len(sys.argv) == 2 and sys.argv[1] == '--test' else False
LISTEN_PORT = 9007 if TEST_MODE else 8080
...
class SBANRFConnection(object):
    ...
    def resource_providing(self):
        ...
        ents.pop(0)  # delete NRF in entities list
        confs = {str(k): str(v) for k, v in body_json['MODE'].items()}
        confs.pop('NRF', None)  # delete NRF in config table
    ...
    def send_response(self):
        body = json.dumps(
            {'Content': 'Recieved by SBAES - NRF'}).encode('utf-8')
        self.conn.send_headers(
            stream_id=self.stream_id,
            headers=[
                (':status', '200'),
                ('server', 's_NRF-h2-server/1.0'),
...
while True:
    try:
        connection = SBANRFConnection(sock.accept()[0], TEST_MODE, LOCK)
