...
# Test Mode:    Listen: localhost:9008
# Otherwise:    Listen: 10.0.3.2:8080
TEST_MODE = True if len(sys.argv) == 2 and sys.argv[1] == '--test' else False
LISTEN_PORT = 9008 if TEST_MODE else 8080
...
def log(text):
    with open("logs/AUSF.txt", "a") as f:
        f.write(str(text) + "\n")


class SBAAUSFConnection(object):
    ...
    def resource_providing(self):
        ...
        log(body_json)
        self.rx_config = str(body_json['MODE']['AUSF'])
        self.fw_body = {
            'RESULT': body_json['RESULT']
        }
        ents = [str(x) for x in body_json['SBA_ENTITY']]
        ents.pop(0)  # delete AUSF in entities list
        confs = {str(k): str(v) for k, v in body_json['MODE'].items()}
        confs.pop('AUSF', None)  # delete AUSF in config table
        ...
        self.fw_body['RESULT']['AUSF'] = True  # Always gives True for testing
        self.lock.acquire()
    ...
    def send_response(self):
        body = json.dumps(
            {'Content': 'Recieved by SBAES - AUSF'}).encode('utf-8')
        self.conn.send_headers(
            stream_id=self.stream_id,
            headers=[
                (':status', '200'),
                ('server', 's_AUSF-h2-server/1.0'),
    ...
    def foward_request(self):
        ...
        log('Fowarded packet to SBA Entity {} at http://{}'.format(next_ent, fw_table[next_ent]))  # Next SBA Entity to foward to
...
log('SBA Entity AMF server started at http://{}:{}'.format(
    '0.0.0.0' if TEST_MODE else '10.0.3.2', LISTEN_PORT))
...
while True:
    try:
        connection = SBAAUSFConnection(sock.accept()[0], TEST_MODE, LOCK)
